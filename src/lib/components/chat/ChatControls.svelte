<script lang='ts'>
  import { mobile, showArtifacts, showCallOverlay, showControls, showOverview } from '$lib/stores'
  ...
  export function openPane() {
    if (Number.parseInt(localStorage?.chatControlsSize)) {
      pane.resize(Number.parseInt(localStorage?.chatControlsSize))
    }
    else {
      pane.resize(minSize)
    }
