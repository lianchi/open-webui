...
export function replaceTokens(content, sourceIds, char, user) {
  const tokens = [
...
export function sanitizeResponseContent(content: string) {
  return content
    .replace(/<\|[a-z]*$/, '')
    .replace(/<\|[a-z]+\|$/, '')
    .replace(/<$/, '')
...
export function convertMessagesToHistory(messages) {
  const history = {
    messages: {},
    currentId: null,
  }
...
export function canvasPixelTest() {
...
export async function compressImage(imageUrl, maxWidth, maxHeight) {
...
export function generateInitialsImage(name) {
  const canvas = document.createElement('canvas')
  const ctx = canvas.getContext('2d')
  canvas.width = 100
  canvas.height = 100
...
export async function copyToClipboard(text, formatted = false) {
  if (formatted) {
    const options = {
      throwOnError: false,
      highlight(code, lang) {
...
export function compareVersion(latest, current) {
...
export function extractCurlyBraceWords(text) {
  const regex = /\{\{([^}]+)\}\}/g
  const matches = []
  let match
...
export function removeLastWordFromString(inputString, wordString) {
...
export function removeFirstHashWord(inputString) {
...
export function getImportOrigin(_chats) {
...
export async function getUserPosition(raw = false) {
...
function convertOpenAIMessages(convo) {
...
export function convertOpenAIChats(_chats) {
...
export function removeFormattings(str: string) {
  return (
    str
...
export function cleanText(content: string) {
  return removeFormattings(removeEmojis(content.trim()))
}

export function removeDetails(content, types) {
...
export function removeAllDetails(content) {
  content = content.replace(/<details[^>]*>.*?<\/details>/gis, '')
  return content
}

export function processDetails(content) {
  ...
  const detailsRegex = /<details\s+type="tool_calls"([^>]*)>([\s\S]*?)<\/details>/gi
...
export function extractSentences(text: string) {
  ...
  sentences = sentences.map((sentence) => {
    ...
    return sentence.replace(/\0(\d+)\0/g, (_, idx) => codeBlocks[idx])
  })

  return sentences.map(cleanText).filter(Boolean)
}

export function extractParagraphsForAudio(text: string) {
  const codeBlocks: string[] = []
  let index = 0
  ...
  paragraphs = paragraphs.map((paragraph) => {
    ...
    return paragraph.replace(/\0(\d+)\0/g, (_, idx) => codeBlocks[idx])
  })

  return paragraphs.map(cleanText).filter(Boolean)
}

export function extractSentencesForAudio(text: string) {
...
export function getMessageContentParts(content: string, split_on: string = 'punctuation') {
...
export function getPromptVariables(user_name, user_location) {
  return {
    '{{USER_NAME}}': user_name,
 ...
 */
export function promptTemplate(template: string,	user_name?: string,	user_location?: string): string {
 ...
 * @returns {string} The template string with the placeholders replaced by the prompt.
 */
export function titleGenerationTemplate(template: string, prompt: string): string {
...
export function approximateToHumanReadable(nanoseconds: number) {
...
export function getTimeRange(timestamp) {
  const now = new Date()
...
export function bestMatchingLanguage(supportedLanguages, preferredLanguages, defaultLocale) {
...
export function createMessagesList(history, messageId) {
  if (messageId === null) {
    return []
  }
...
export function convertOpenApiToToolPayload(openApiSpec) {
  const toolPayload = []
