<script lang='ts'>
  ...
  const submitPrompt = async (userPrompt: string, { _raw = false } = {}) => {
    ...
    if (messages.length !== 0 && messages.at(-1).error && !messages.at(-1).content) {
      // Error in response
      toast.error('糟糕！有一个错误出现在了之前的回复中。')
      return
    }
    if (
      files.length > 0
...
>
  {#if chatIdProp === '' || (!loading && chatIdProp)}
    {#if $settings?.backgroundImageUrl ?? null}
      ...
      <div
        class='absolute top-0 left-0 w-full h-full bg-linear-to-t from-white to-white/80 dark:from-gray-900 dark:to-gray-900/90 z-0'
      />
    {/if}
              ...
              {#if ($config?.license_metadata?.seats ?? null) !== null && $config?.user_count > $config?.license_metadata?.seats}
                <Banner
                  banner={{
                    type: 'error',
                    title: '许可证错误',
                    content: '已达到最大授权人数，请联系支持人员提升授权人数。',
                  }}
                />
              {/if}
